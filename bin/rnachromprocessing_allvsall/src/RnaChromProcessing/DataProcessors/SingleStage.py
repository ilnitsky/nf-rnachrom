...
class SingleStageProcessor(BaseProcessor):
    ...
    def setup_stage(self, cfg: Dict[str, Any]):
        ...
        if self.stage == 'rsites':
            ...
            contacts_default_cfg.update(cfg.get('stats', {}))
            self.stage_runner = StatsCalc(contacts_default_cfg,self.output_dir, self.cpus, self.stats,
                  self.stats_prefix, self.dna_ids, self.rna_ids)
        else:

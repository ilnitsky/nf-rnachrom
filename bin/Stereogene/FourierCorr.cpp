...
char *resFileName(const char* n1,const char* n2){
	char b[10000],nam1[4096],nam2[4096];
	strcpy(nam1,n1); strcpy(nam2,n2);
	snprintf(b,sizeof(b),"%s~%s",nam1,nam2);
	return strdup(b);
}
...
char * makeOutFilename(char * prof1, char*prof2){
	...
	if(aliases){
		char p1F[4096], p2F[4096]; strcpy(p1F,p1Fname); strcpy(p2F,p2Fname);

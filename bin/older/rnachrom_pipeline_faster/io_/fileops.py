...
def load_BED(path: str, names = annot_BED, dtype = None) -> pd.DataFrame:
    ...
    if names == BED3:
        ...
        return pd.read_csv(path, header = None, index_col=None, sep = '\t', usecols = [0,1,2,3,4,5], names = names, dtype = dtype)
    else:
...
def load_rdc(path: str, header = None, names = [], use_cols = False, dtype = None) -> pd.DataFrame:
    ...
    if header == 0 and len(names) == 0:
        ...
        rdc = pd.read_csv(path, header=header, index_col=False, sep='\t', dtype = dtype)
    elif not use_cols:
        rdc = pd.read_csv(path, header=header, index_col=False, sep='\t', names=names, dtype = dtype)
    else:
        rdc = pd.read_csv(path, header=header, index_col=False, sep='\t', names=names, usecols = use_cols, dtype = dtype)
    return rdc

...
def splicing_cases(df: pd.DataFrame) -> pd.DataFrame:
    """
    Count the N, I, D occurrences in the CIGAR string.
    """
...
def process_simple_splicing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter out 'S' (soft-clipping)
    pattern = re.compile(r'(\d+)[MIDNSHPX=]')
    df_ss['N1'] = df_ss['rna_cigar'].apply(lambda x: int(pattern.findall(x)[0]))
    df_ss['N2'] = df_ss['rna_cigar'].apply(lambda x: int(pattern.findall(x)[2]))

    # Recalculate start and end coordinates based on the larger part
